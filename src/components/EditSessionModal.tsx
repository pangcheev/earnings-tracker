...
export function EditSessionModal({ session, isOpen, onClose, onSave }: EditSessionModalProps) {
  ...
  // Always Halo (Soul Bridge removed)
  const isHalo = true
  ...
  const totalEarnings = services.reduce((acc, s) => acc + (s.haloBasePrice || s.rate), 0)
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave() }} className="space-y-6">
          ...
          <div className="border-t border-slate-600 pt-6">
            ...
            <button
              ...
              className="w-full text-white px-4 py-2 rounded font-semibold transition-colors bg-blue-700 hover:bg-slate-600"
            >
              + Add Service
            </button>
